...
void BaliBali::onStart()
{
    ...
    if (buf[1] == '1')
        Broodwar->enableFlag(Flag::CompleteMapInformation); // Note: Fog of War remains
    logCommands = (buf[2] == '1');
...
void handleCommand(int command, int unitID, int arg0, int arg1, int arg2)
{
    ...
    case 1:
        if (logCommands)
            Broodwar->sendText("Unit:%d attackMove(%d, %d)",unitID, arg0, arg1);
    ...
    case 2:
        ...
        {
            if (logCommands)
                Broodwar->sendText("Unit:%d attackUnit(%d)", unitID, arg0);
    ...
    case 3:
        if (logCommands)
            Broodwar->sendText("Unit:%d rightClick(%d, %d)",unitID, arg0, arg1);
    ...
    case 4:
        ...
        {
            if (logCommands)
                Broodwar->sendText("Unit:%d rightClick(%d)", unitID, arg0);
    ...
    case 5:
        ...
        { // NULL doesnt work here, NULL = 0, Terran_Marine=0
            Broodwar->sendText("Invalid Command, Unit:%d train(%d)", unitID, arg0);
        }
        else
        {
            if (logCommands)
                Broodwar->sendText("Unit:%d train(%d)", unitID, arg0);
    ...
    case 6:
        ...
        {
            if (logCommands)
                Broodwar->sendText("Unit:%d build(%d, %d, %d)", unitID, arg0, arg1, arg2);
    ...
    case 7:
        ...
        {
            if (logCommands)
                Broodwar->sendText("Unit:%d buildAddon(%d)", unitID, arg0);
    ...
    case 8:
        ...
        {
            if (logCommands)
                Broodwar->sendText("Unit:%d research(%d)", unitID, arg0);
    ...
    case 9:
        ...
        {
            if (logCommands)
                Broodwar->sendText("Unit:%d upgrade(%d)", unitID, arg0);
    ...
    case 10:
        if (logCommands)
            Broodwar->sendText("Unit:%d stop()", unitID);
        unit->stop();
        break;
        // virtual bool holdPosition() = 0;
    case 11:
        if (logCommands)
            Broodwar->sendText("Unit:%d holdPosition()", unitID);
        unit->holdPosition();
        break;
        // virtual bool patrol(Position position) = 0;
    case 12:
        if (logCommands)
            Broodwar->sendText("Unit:%d patrol(%d, %d)", unitID, arg0, arg1);
    ...
    case 13:
        ...
        {
            if (logCommands)
                Broodwar->sendText("Unit:%d follow(%d)", unitID, arg0);
    ...
    case 14:
        if (logCommands)
            Broodwar->sendText("Unit:%d setRallyPosition(%d, %d)", unitID, arg0, arg1);
    ...
    case 15:
        ...
        {
            if (logCommands)
                Broodwar->sendText("Unit:%d setRallyUnit(%d)", unitID, arg0);
    ...
    case 16:
        ...
        {
            if (logCommands)
                Broodwar->sendText("Unit:%d repair(%d)", unitID, arg0);
    ...
    case 17:
        ...
        {
            if (logCommands)
                Broodwar->sendText("Unit:%d morph(%d)", unitID, arg0);
    ...
    case 18:
        if (logCommands)
            Broodwar->sendText("Unit:%d burrow()", unitID);
        unit->burrow();
        break;
        // virtual bool unburrow() = 0;
    case 19:
        if (logCommands)
            Broodwar->sendText("Unit:%d unburrow()", unitID);
        unit->unburrow();
        break;
        // virtual bool siege() = 0;
    case 20:
        if (logCommands)
            Broodwar->sendText("Unit:%d siege()", unitID);
        unit->siege();
        break;
        // virtual bool unsiege() = 0;
    case 21:
        if (logCommands)
            Broodwar->sendText("Unit:%d unsiege()", unitID);
        unit->unsiege();
        break;
        // virtual bool cloak() = 0;
    case 22:
        if (logCommands)
            Broodwar->sendText("Unit:%d cloak()", unitID);
        unit->cloak();
        break;
        // virtual bool decloak() = 0;
    case 23:
        if (logCommands)
            Broodwar->sendText("Unit:%d decloak()", unitID);
        unit->decloak();
        break;
        // virtual bool lift() = 0;
    case 24:
        if (logCommands)
            Broodwar->sendText("Unit:%d lift()", unitID);
        unit->lift();
        break;
        // virtual bool land(TilePosition position) = 0;
    case 25:
        if (logCommands)
            Broodwar->sendText("Unit:%d land(%d, %d)", unitID, arg0, arg1);
    ...
    case 26:
        ...
        {
            if (logCommands)
                Broodwar->sendText("Unit:%d load(%d)", unitID, arg0);
    ...
    case 27:
        ...
        {
            if (logCommands)
                Broodwar->sendText("Unit:%d unload(%d)", unitID, arg0);
    ...
    case 28:
        if (logCommands)
            Broodwar->sendText("Unit:%d unloadAll()", unitID);
        unit->unloadAll();
        break;
        // virtual bool unloadAll(Position position) = 0;
    case 29:
        if (logCommands)
            Broodwar->sendText("Unit:%d unloadAll(%d, %d)", unitID, arg0, arg1);
        unit->unloadAll(getPosition(arg0, arg1));
        break;
        // virtual bool cancelConstruction() = 0;
    case 30:
        if (logCommands)
            Broodwar->sendText("Unit:%d cancelConstruction()", unitID);
        unit->cancelConstruction();
        break;
        // virtual bool haltConstruction() = 0;
    case 31:
        if (logCommands)
            Broodwar->sendText("Unit:%d haltConstruction()", unitID);
        unit->haltConstruction();
        break;
        // virtual bool cancelMorph() = 0;
    case 32:
        if (logCommands)
            Broodwar->sendText("Unit:%d cancelMorph()", unitID);
        unit->cancelMorph();
        break;
        // virtual bool cancelTrain() = 0;
    case 33:
        if (logCommands)
            Broodwar->sendText("Unit:%d cancelTrain()", unitID);
        unit->cancelTrain();
        break;
        // virtual bool cancelTrain(int slot) = 0;
    case 34:
        if (logCommands)
            Broodwar->sendText("Unit:%d cancelTrain(%d)", unitID, arg0);
        unit->cancelTrain(arg0);
        break;
        // virtual bool cancelAddon() = 0;
    case 35:
        if (logCommands)
            Broodwar->sendText("Unit:%d cancelAddon()", unitID);
        unit->cancelAddon();
        break;
        // virtual bool cancelResearch() = 0;
    case 36:
        if (logCommands)
            Broodwar->sendText("Unit:%d cancelResearch()", unitID);
        unit->cancelResearch();
        break;
        // virtual bool cancelUpgrade() = 0;
    case 37:
        if (logCommands)
            Broodwar->sendText("Unit:%d cancelUpgrade()", unitID);
        unit->cancelUpgrade();
        break;
    ...
    case 38:
        ...
        {
            if (logCommands)
                Broodwar->sendText("Unit:%d useTech(%d)", unitID, arg0);
    ...
    case 39:
        ...
        {
            if (logCommands)
                Broodwar->sendText("Unit:%d useTech(%d, %d, %d)", unitID, arg0, arg1, arg2);
    ...
    case 40:
        ...
        {
            if (logCommands)
                Broodwar->sendText("Unit:%d useTech(%d, %d)", unitID, arg0, arg1);
...
void BaliBali::onUnitCreate(Unit* unit)
{
 ...
 */
void BaliBali::onUnitDestroy(BWAPI::Unit* unit)
{
    int key = unitMap.erase(unit);
    unitIDMap.erase(key);
}

bool BaliBali::onSendText(std::string text)
{
    return true;
}
